...
byte Pot::getValue()
{
  ...
  if (tmp >= 8 || tmp <= -8) // Estos 2 valores es el filtro de ruido o sensibilidad, a mas, mas filtro. These 2 values is the noise filter or sensitivity, more, more filter.
  {
    _oldValue = _value;
      ...
      _retvalue = map(_value, 0, 1015, 0, 127); // Mapeo, Mapping   Original= 1023 
...
//  int _retvalue;
  if (tmp >= 8 || tmp <= -8) // Estos 2 valores es el filtro de ruido o sensibilidad, a mas, mas filtro. These 2 values is the noise filter or sensitivity, more, more filter.
  {
    ...
    if(_bits == 7)
      _retvalue = map(_value, 0, 1023, 0, 127);  // 1023
     
    if(_bits == 14)
      _retvalue = map(_value, 2, 1023, 0, 16383); // 1023
    
    if(_bits == 100)
      _retvalue = map(_value, 0, 1023, -8192, 8191); // 1023
...
bool Pot::potec2(bool parte, byte modificar, int min, int max)
{
    ...
    if (tmp >= 8 || tmp <= -8) // Estos 2 valores es el filtro de ruido o sensibilidad, a mas, mas filtro. These 2 values is the noise filter or sensitivity, more, more filter.
    {
      ...
      const long rangedValue = map(_value, 0, 1015, min, max);
...

{
    ...
    if (tmp >= 8 || tmp <= -8) // Estos 2 valores es el filtro de ruido o sensibilidad, a mas, mas filtro. These 2 values is the noise filter or sensitivity, more, more filter.
    {
     ...
     _retvalue = map(_value, 0, 1015, min, max);   
...
bool Pot::potec7(bool parte, byte modificar, int min, int max)
{
    // FILTRADO DE VALOR ANALOGICO POR UMBRAL.  ANALOG VALUE FILTERING BY THRESHOLD
    _value = analogRead(_pin); // modificar pin de lectura
    int _retvalue;
    ...
    if (tmp >= 8 || tmp <= -8) // Estos 2 valores es el filtro de ruido o sensibilidad, a mas, mas filtro. These 2 values is the noise filter or sensitivity, more, more filter.
    {
        ...
        byte lowByte = _retvalue & 0x7F; // 7 bits inferiores
        byte highByte = (_retvalue >> 7) & 0x03; // 2 bits superiores

        // Colocar los bytes en las posiciones 15 y 16 del array
        Potec2[modificar - 1] = highByte;
        Potec2[modificar] = lowByte;
